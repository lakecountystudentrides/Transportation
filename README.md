...
- `owner-trips.js` — powers the "Today's Roster" section on `manage-drivers.html`: every trip active today, split into drop-off/pickup like the driver dashboard's roster but with full trip detail on each card; owner session only
