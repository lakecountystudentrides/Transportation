...
- `trips.js` / `trip-status.js` — power the driver dashboard; pressing Start Trip or Arrived emails the parent via Resend (`functions/_lib/email.js`). Accepts either the owner's master code or an individual driver's code (`functions/_lib/driverAuth.js`)
- `drivers.js` / `driver-deactivate.js` — add, list, and deactivate individual driver access codes; owner-only (`manage-drivers.html`, gated by `DRIVER_ACCESS_TOKEN` exactly, not an individual driver's code)
